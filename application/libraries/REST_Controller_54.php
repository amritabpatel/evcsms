...
abstract class REST_Controller extends CI_Controller {
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        if (! method_exists($this, $controller_method))
        {
            $response_arr=array( $this->config->item('rest_status_field_name') => FALSE,
                    $this->config->item('rest_message_field_name') => $this->lang->line('text_rest_unknown_method'));
            
            $this->response($response_arr, self::HTTP_METHOD_NOT_ALLOWED);
            $is_valid_request = false;
        }
     ...
     */
    public function response($data = NULL, $http_code = NULL,$saveRequestToserver_arr=NULL)
    {           
        // print_r($data);
         
        ## *****Inci_Code ******* Add Request and Rsponse On Our Server  Start********
        if($saveRequestToserver_arr != NULL && $saveRequestToserver_arr["is_request"]){
            $header=array("Content-Type"=>"application/json; charset=utf-8");
            $response=(isset($data["success"]))?$data:array();
            $error=(isset($data["error"]))?$data:array();
            $requets_array=array("project_id"=>$saveRequestToserver_arr["project_id"],"method"=>$saveRequestToserver_arr["method"],"method_url"=>$saveRequestToserver_arr["method_url"],"method_type"=>$saveRequestToserver_arr["method_type"],"header"=>$header,"request"=>$saveRequestToserver_arr["request"],"response"=>$response,"errors"=>$error,"notes"=>$saveRequestToserver_arr["notes"]);
            $reuslt=$this->inci_SaveRequestOnServer($requets_array);
           
        }
        ## *****Inci_Code ******* Add Request and Rsponse On Our Server  End********
        ob_start();
...
//        $post = json_decode($jsonInput,true);
   ...
   ## ****Inci_Code *** Function to save requst on our server Start ********* 
    protected function inci_SaveRequestOnServer($array){
        
        if(isset($array) && !empty($array)){
				$json_data=$array;
				$url=CUSTOMER_PROJECT_URL;
   ...
   ## ****Inci_Code *** Function to save requst on our server End *********** 
}
